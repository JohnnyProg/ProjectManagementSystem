...
public class CommentController {
    ...
    @GetMapping("/{task_id}")
    public ResponseEntity<List<CommentDto>> getAllCommentsForTask(@PathVariable long task_id) throws ResourceNotFoundException {
        List<Comment> comments = commentService.getAllForTask(task_id);
        return new ResponseEntity<>(commentListToDtos(comments), HttpStatus.OK);
    }

    @PostMapping("/")
    public ResponseEntity<CommentDto> createComment(@RequestBody CreateCommentDto comment) throws ResourceNotFoundException {
